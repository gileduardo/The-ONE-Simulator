...
import routing.greg.Algorithm;

/**
 * Implementation of GREG router by Gil Eduardo de Andrade
*/
public class GregRouter extends ActiveRouter {

	/** Context router's setting namespace ({@value})*/ 
	public static final String GREG_NS = "GregRouter";
	...
	public GregRouter(Settings s) {

		super(s);
		Settings GregSettings = new Settings(GREG_NS);

		this.useReplication = GregSettings.getBoolean(REPLICATION_MODE);
		System.out.println("[GREG] - Modo de Replicação: " + useReplication);
	
		this.nrReplics = GregSettings.getInt(NR_REPLICS);
		System.out.println("[GREG] - Número de Réplicas: " + nrReplics);

		this.timeOld = GregSettings.getDouble(TIME_OLD);
		System.out.println("[GREG] - Tempo Informação Obsoleta: " + timeOld + " segundos");

		this.timeMax = GregSettings.getDouble(TIME_MAX);
		System.out.println("[GREG] - Tempo Máximo para Previsão: " + timeMax + " segundos");
	 ...
	 */
	protected GregRouter(GregRouter r) {
		super(r);
	...
	public MessageRouter replicate() {
		return new GregRouter(this);
	}

	// Cria a Tabela de Contexto dos Últimos Contatos Efetuados
	public void initGregRouter() {
		this.objAlg = new Algorithm(this.nrReplics);
