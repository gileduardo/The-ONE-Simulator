...
import routing.velosentplus.Algorithm;
import routing.velosentplus.ConMsg;

/**
 * Implementation of CONTEXT router by Gil Eduardo de Andrade
*/
public class VeloSentPlusRouter extends ActiveRouter {

	/** Context router's setting namespace ({@value})*/ 
	public static final String VELOSENTPLUS_NS = "VeloSentPlusRouter";

	/** identifier for the binary-mode setting ({@value})*/ 
	public static final String REPLICATION_MODE = "ReplicationMode";
	/** identifier for the binary-mode setting ({@value})*/ 
	public static final String CONTACT_WINDOW = "useContactWindow";
	...
	/** Indica se deve usar o novo método baseado na janela do contato e tamanho do pacote*/
	private boolean useContactWindow;
	

	public VeloSentPlusRouter(Settings s) {
		super(s);
		Settings VeloSentPlusSettings = new Settings(VELOSENTPLUS_NS);
		useReplication = VeloSentPlusSettings.getBoolean(REPLICATION_MODE);
		useContactWindow = VeloSentPlusSettings.getBoolean(CONTACT_WINDOW);
		// setReplyMsg(useReplication);
		initVeloSentPlusRouter();
	}
	 ...
	 */
	protected VeloSentPlusRouter(VeloSentPlusRouter r) {
		super(r);
		this.useReplication = r.useReplication;
		this.useContactWindow = r.useContactWindow;
		// setReplyMsg(useReplication);
		initVeloSentPlusRouter();
	}
	...
	public MessageRouter replicate() {
		return new VeloSentPlusRouter(this);
	}

	// Cria a Tabela de Contexto dos Últimos Contatos Efetuados
	public void initVeloSentPlusRouter() {
	...
	public void updateMessagesConnections(Message m, double age, TCUCE d_tcuce) {
		...
		// Calcula o tempo mínimo para o sistema alcançar o que ser busca pelo algoritmo (depende da versão)
		double time;

		// Tempo mínimo, usada da determinar o próximo nó a receber a mensagem m
		double min_time = -1;
		...
		objAlg.setDataNeighbor(node.getPosX(), node.getPosY(), node.getSpeedX(), node.getSpeedY(), node.getRange(), node.getTransmitSpeed());
		// Informa o Tamanho da Mensagem - (VERSÃO 2.1)
		objAlg.setMsgSize(m.getSize());

		// RODA O ALGORITMO DE CONTEXTO
		// Calcula tempo mínimo final para o host de posse da mensagem (nó âncora) e
		// considera como o melhor inicialmente, até encontrar um vizinho com maior possibilidade de entrega

		// (VERSÃO 2.1) Considerando Janela de Contato - Tamanho da Mensagem - Taxa de Transmissão
		if(this.useContactWindow) {
			// Calcula o tempo mínimo para o destino entrar no raio de alcance do nó âncora atual
			time = objAlg.algorithmRunCW();
		}
		// (VERSÃO 2.0) Considerando apenas o menor Tempo Mínimo 
		else {
			// Calcula o tempo mínimo para o sistema alcançar a distância minima entre o nó âncora atual e o destino
			time = objAlg.algorithmRun();
		}
		...
		if(connections != null) {
				...
				objAlg.setDataNeighbor(node.getPosX(), node.getPosY(), node.getSpeedY(), node.getSpeedY(), node.getRange(), node.getTransmitSpeed());

				// RODAR O ALGORÍTMO DE CONTEXTO
				// (VERSÃO 2.1)
				if(this.useContactWindow) {
					min_time = objAlg.algorithmRunCW();
				}
				// (VERSÃO 2.0)
				else {
					min_time = objAlg.algorithmRun();
				}

				
				// (VERSÃO 2.1)	
				// Vizinho possui melhor possibilidade de entrega (menor tempo até o destino entrar no seu
				// raio de alcance) que o nó atual armazena sua conexão para atualizar a lista 
				// de Mensagem/Conexão
				// -------------------------------------------------------------------------
				// (VERSÃO 2.0)
				// Vizinho possui melhor possibilidade (menor tempo até a atingir a distância mínima) 
				// de entrega que o nó atual armazena sua conexão para atualizar a lista de Mensagem/Conexão
				if((min_time < time && min_time != -1) || 
				 	(min_time > -1 && time == -1)) {

					final_conn = conn;					
					// distance = final_distance; // atualiza a melhor distância
					time = min_time; // atualiza o melhor tempo mínimo
					flag = true;
				}
			}
		}
	...
	public void transferDone(Connection con) {
		...
		if (m != null && useReplication == false) {
			...
			// Remove Mensagem - Lista VeloSenPlus
			this.objConMsg.removeMessage(m);
...
/** FUNCIONAMENTO DO "ContextRouter"
	...
	-> Primeira Etapa: 	no momento em que há uma mudança em uma conexão, mais exatamente quando existe uma 
		...
		8)	Quando o método "update()" é invocado ele consulta a lista de mensagens e
			...
			conexão/vizinho na lista a mensagem não é repassada. 
