...
public abstract class ActiveRouter extends MessageRouter {
	...
	// ============================ //
	// GIL EDUARDO DE ANDRADE - GEA //	
	// ============================ //
	protected int tryMessagesForYourConnection(Message m, Connection con) {

		// Verifica se a conexão ainda está ativa
		if(con.isUp()) {
	
			// Atualiza o número de Cópias antes do Envio
			if(m.getNrCopies() > 0) { m.setNrCopies(m.getNrCopies()/2); }
