...
public class Path  {
	...
	// =============================================== //
	// ===== MÉTODOS PARA OBTER A POSIÇÃO FUTURA ===== //
	// =============================================== //
	// Obtém o próximo cruzamento
	public Coord getNextCrossroad() {

		Coord cross = null;

		if(this.coords.size() > 0) {

			if((nextWpIndex+3) < this.coords.size()) {
				cross =  this.coords.get((nextWpIndex+3));
			}
			else {
				cross = this.coords.get((nextWpIndex));
			}
		}
	...
	// Obtém o cruzamento mais próximo da posição estimada
	public Coord getCloserCrossroad(double pex, double pey) {
		
		Coord cr, cross;		
		double aux, menor, X, Y;

		if(this.coords.size() > 0) {

			cross = this.coords.get(0);
			X = (pex - cross.getX());
			Y = (pey - cross.getY());
			menor = Math.sqrt((X*X) + (Y*Y));

			for(int a=1; a<this.coords.size(); a++) {

				cr = this.coords.get(a);
				X = (pex - cr.getX());
				Y = (pey - cr.getY());
